...
def get_user_fairness_loss(y_pred, y_true, genders):
    ...
    mse_male = F.mse_loss(y_pred[mask_male].view(-1), y_true[mask_male].view(-1)) if mask_male.any() else torch.tensor(0.0).to(y_pred.device)
    mse_female = F.mse_loss(y_pred[mask_female].view(-1), y_true[mask_female].view(-1)) if mask_female.any() else torch.tensor(0.0).to(y_pred.device)
...
def get_item_fairness_loss(y_pred, y_true, is_popular):
    ...
    mse_pop = F.mse_loss(y_pred[mask_pop].view(-1), y_true[mask_pop].view(-1)) if mask_pop.any() else torch.tensor(0.0).to(y_pred.device)
    mse_unpop = F.mse_loss(y_pred[mask_unpop].view(-1), y_true[mask_unpop].view(-1)) if mask_unpop.any() else torch.tensor(0.0).to(y_pred.device)
